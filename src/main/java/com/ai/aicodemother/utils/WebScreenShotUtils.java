...
public class WebScreenShotUtils {
    ...
    // 全局静态初始化，通过静态代码块初始化静态变量 WebDriver，避免重复初始化驱动程序
    static {
     ...
     *
     * @param webUrl 网页 URL
     * @return 压缩后的截图文件路径，失败的话返回 null
     */
    public static String saveWebPageScreenShot(String webUrl) {
        // 非空校验
        if (StrUtil.isBlank(webUrl)) {
            log.error("截图失败，网页 URL 为空");
            return null;
        }
        try {
            // 创建临时目录，临时目录中同时保存原始图片和压缩图片
            String rootPath = System.getProperty("user.dir") + "/tmp/screenshots/" + UUID.randomUUID().toString().subSequence(0, 8);
            ...
            // 等待网页加载
            webForPageLoad(webDriver);

            // 截图
            byte[] screenshot = ((TakesScreenshot) webDriver).getScreenshotAs(OutputType.BYTES);
            // 保存到刚才生成的图片路径下
            saveImage(screenshot, imageSavePath);
            log.info("截图成功，保存路径: {}", imageSavePath);
            ...
            log.info("压缩图片成功，保存路径: {}", compressedImagePath);
     ...
     */
    private static void webForPageLoad(WebDriver webDriver) {
        try{
            // 创建等待页面加载对象，最多等待网页渲染 10 秒钟
